<?php
if (!class_exists('sparkconstructionlite_Welcome')) :
    class sparkconstructionlite_Welcome {
		...
		/**
		 * @var sparkconstructionlite_Demo_Importer
		 *
		 * @since 1.1.0
		 */
		private $plugin = null;
		 ...
		 *
		 */
		private static function sparkconstructionlite_redirect( $url ) {
        ...
        */
        public function __construct() {
            ...
            add_action('wp_ajax_sparkconstructionlite_activate_plugin', array($this, 'activate_plugin'));

			add_action( 'wp_ajax_thim_core_installer', array( $this, 'sparkconstructionlite_ajax_install' ) );
			
			add_action( 'sparkconstructionlite_installer_step_start', array( $this, 'sparkconstructionlite_prepare_installation' ) );
			add_action( 'sparkconstructionlite_installer_step_install', array( $this, 'sparkconstructionlite_step_install' ) );
			add_action( 'sparkconstructionlite_installer_step_activate', array( $this, 'sparkconstructionlite_step_activate' ) );
        }

        /** Trigger Welcome Message Notification */
        public function admin_notice($hook) {
            $hide_notice = get_option('sparkconstructionlite_hide_notice');
            if (!$hide_notice) {
        ...
        public function admin_notice_content() {
                ...
                if ('appearance_page_sparkconstructionlite-welcome' === $screen->id || (isset($screen->parent_file) && 'plugins.php' === $screen->parent_file && 'update' === $screen->id) || 'theme-install' === $screen->id) {
                    return;
                }
                $slug = $filename = 'elementor';
            ?>
                <div class="updated notice sparkconstructionlite-welcome-notice">
                    <div class="sparkconstructionlite-welcome-notice-wrap">
                        <h2><?php esc_html_e('Congratulations!', 'spark-construction-lite'); ?></h2>
                        <p><?php printf(esc_html__('%1$s is now installed and ready to use. You can start either by importing the ready-made demo or get started by customizing it your self.', 'spark-construction-lite'), $this->theme_name); ?></p>
                        <div class="sparkconstructionlite-welcome-info">
                            <div class="sparkconstructionlite-welcome-thumb">
                                <img src="<?php echo esc_url(get_stylesheet_directory_uri() . '/screenshot.png'); ?>" alt="<?php echo esc_attr__('Spark Construction Lite Demo', 'spark-construction-lite'); ?>">
                            </div>
                            <div class="sparkconstructionlite-welcome-import">
                                <h2><?php esc_html_e('Import Demo', 'spark-construction-lite'); ?></h2>
                                <p><?php esc_html_e('Click below to install and activate the Ikreate Demo Importer Plugin.', 'spark-construction-lite'); ?></p>
                                <p><?php echo $this->sparkconstructionlite_setup_content(); ?></p>
                            </div>
                            <div class="sparkconstructionlite-welcome-getting-started">
                                <h2><?php esc_html_e('Get Started', 'spark-construction-lite'); ?></h2>
                                <p><?php printf(esc_html__('Here you will find all the necessary features, and information about %s theme.', 'spark-construction-lite'), $this->theme_name); ?></p>
                                <p><a href="<?php echo esc_url(admin_url('admin.php?page=sparkconstructionlite-welcome')); ?>" class="button button-primary"><span class="dashicons dashicons-admin-settings"></span> <?php esc_html_e('Go to Setting Page', 'spark-construction-lite'); ?></a></p>
                            </div>
                        </div>
                        <a href="<?php echo wp_nonce_url(add_query_arg('sparkconstructionlite_hide_notice', 1), 'sparkconstructionlite_hide_notice_nonce', '_sparkconstructionlite_notice_nonce'); ?>" class="notice-close"></a>
                    </div>
                </div>
            <?php 
        }
        ...
        public function welcome_styles_and_scripts($hook) {
            if ('theme-install.php' !== $hook) {
                $importer_params = array(
                    'installing_text' => esc_html__('Installing Demo Importer Plugin', 'spark-construction-lite'),
                    'activating_text' => esc_html__('Activating Demo Importer Plugin', 'spark-construction-lite'),
                    'importer_page' => esc_html__('Go to Demo Importer Page', 'spark-construction-lite'),
                    'importer_url' => admin_url('themes.php?page=elementor'),
                    'error' => esc_html__('Error! Reload the page and try again.', 'spark-construction-lite'),
                );
                wp_enqueue_style('sparkconstructionlite-welcome', get_template_directory_uri() . '/inc/welcome/css/welcome.css', array());
                wp_enqueue_script('sparkconstructionlite-welcome', get_template_directory_uri() . '/inc/welcome/js/welcome.js', array('plugin-install', 'updates'), '', true);
                wp_localize_script('sparkconstructionlite-welcome', 'importer_params', $importer_params);
            }
        }
        ...
        public function admin_footer_text($text) {
            $screen = get_current_screen();
            if ('toplevel_page_sparkconstructionlite-welcome' == $screen->id) {
                $text = sprintf(esc_html__('Please leave us a %s rating if you like our theme . A huge thank you from ikreate themes in advance!', 'spark-construction-lite'), '<a href="https://wordpress.org/support/theme/spark-construction-lite/reviews/?filter=5#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>');
            }
            return $text;
        }

        public function erase_hide_notice() {
            delete_option('sparkconstructionlite_hide_notice');
        }

		private function sparkconstructionlite_get_thim_core() {
			if ( $this->plugin === null ) {
				$plugin = new sparkconstructionlite_Plugin();
		...
		private function sparkconstructionlite_get_link_step( $step = '' ) {
			return add_query_arg( array( 'step' => $step ) );
		}

		public function sparkconstructionlite_ajax_install() {
            
			$plugin = $this->sparkconstructionlite_get_thim_core();
		...
		private function sparkconstructionlite_setup_content() {
			
			$thim_core   = $this->sparkconstructionlite_get_thim_core();
			$status      = $thim_core->get_status();
			$step        = isset( $_REQUEST['step'] ) ? $_REQUEST['step'] : 'start';

			do_action( "sparkconstructionlite_installer_step_$step" );
		}
		 ...
		 * @since 1.1.0
		 */
		public function sparkconstructionlite_prepare_installation() {
			...
			$link = $this->sparkconstructionlite_get_link_step( $step );
			
			if ( $status == 'inactive' ) {
				$import_class = 'button button-primary';
				$import_button_text = esc_html__('Activate Demo Importer Plugin', 'spark-construction-lite');
			} elseif ( $status == 'active' ) {
				$import_class = 'button button-primary';
				$import_button_text = esc_html__('Go to Demo Importer Page', 'spark-construction-lite');
				$link =  admin_url( '/themes.php?page=ikreate-demo-importer' );
			} else {
				$import_class = 'thim-button-link button button-primary';
				$import_button_text = esc_html__('Install Demo Importer Plugin', 'spark-construction-lite');	
			}
		 ...
		 * @since 1.1.0
		 */
		public function sparkconstructionlite_step_install() {

			$link = $this->sparkconstructionlite_get_link_step( 'activate' );
			...
			if ( $status != 'not_installed' ) {
				self::sparkconstructionlite_redirect( $link );
			}
            ...
            ?>
                <a class="button button-primary" data-href="<?php echo esc_url( $link ) ?>" href=<?php echo esc_url( $link ) ?>>
					<?php esc_html_e('Activate Demo Importer Plugin', 'spark-construction-lite') ?>
				</a>
            <?php
			if ( $result ) {
				self::sparkconstructionlite_redirect( $link );
		 ...
		 * @since 1.1.0
		 */
		public function sparkconstructionlite_step_activate() {

			$plugin = $this->sparkconstructionlite_get_thim_core();

			if ( $plugin->activate( true ) || $plugin->is_active() ) {

				$this->sparkconstructionlite_redirect_dashboard();
		 ...
		 * @since 1.1.0
		 */
		private function sparkconstructionlite_redirect_dashboard() {

			$url = admin_url( 'themes.php?page=ikreate-demo-importer' );

			self::sparkconstructionlite_redirect( $url );
		}

    }
    new sparkconstructionlite_Welcome();
endif;
