jQuery(document).ready(function ($) {
    ...
    $('body').on('click', '.sparkconstructionlite-customizer-icon-box .sparkconstructionlite-icon-list li', function () {
        var icon_class = $(this).find('i').attr('class');
        $(this).closest('.sparkconstructionlite-icon-box').find('.sparkconstructionlite-icon-list li').removeClass('icon-active');
        $(this).addClass('icon-active');
        $(this).closest('.sparkconstructionlite-icon-box').prev('.sparkconstructionlite-selected-icon').children('i').attr('class', '').addClass(icon_class);
        $(this).closest('.sparkconstructionlite-icon-box').next('input').val(icon_class).trigger('change');
        $(this).closest('.sparkconstructionlite-icon-box').slideUp();
    });

    $('body').on('click', '.sparkconstructionlite-customizer-icon-box .sparkconstructionlite-selected-icon', function () {
        $(this).next().slideToggle();
    });

    $('body').on('change', '.sparkconstructionlite-customizer-icon-box .sparkconstructionlite-icon-search select', function () {
        var selected = $(this).val();
        $(this).closest('.sparkconstructionlite-icon-box').find('.sparkconstructionlite-icon-search-input').val('');
        $(this).closest('.sparkconstructionlite-icon-box').find('.sparkconstructionlite-icon-list li').show();
        $(this).closest('.sparkconstructionlite-icon-box').find('.sparkconstructionlite-icon-list').hide().removeClass('active');
        $(this).closest('.sparkconstructionlite-icon-box').find('.' + selected).fadeIn().addClass('active');
    });

    $('body').on('keyup', '.sparkconstructionlite-customizer-icon-box .sparkconstructionlite-icon-search input', function (e) {
        var $input = $(this);
        var keyword = $input.val().toLowerCase();
        search_criteria = $input.closest('.sparkconstructionlite-icon-box').find('.sparkconstructionlite-icon-list.active i');
    ...
    $('.customize-control-range').each(function () {
        ...
        var newSlider = $(this).find('.sparkconstructionlite-slider');
    ...
    $('.customize-control-range .sparkconstructionlite-slider').on('slide', function (event, ui) {
    ...
    $('.customize-control-range .slider-reset').on('click', function () {
        ...
        $(this).parents('.customize-control-range').find('.sparkconstructionlite-slider').slider('value', resetValue);
    });

    // Update slider if the input field loses focus as it's most likely changed
    $('.customize-control-range .slider-input').blur(function () {
        var resetValue = $(this).val();
        var slider = $(this).parents('.customize-control-range').find('.sparkconstructionlite-slider');
    ...
    // Linked button
    $('.sparkconstructionlite-linked').on('click', function () {
    ...
    // Unlinked button
    $('.sparkconstructionlite-unlinked').on('click', function () {
...
(function (api) {
    ...
    // Extends our custom "sparkconstructionlite" section.
    ...
    api.sectionConstructor['sparkconstructionlite-upgrade-section'] = api.Section.extend({
        // No events for this type of section.
        attachEvents: function () { },
        // Always make the section active.
        isContextuallyActive: function () {
    ...
    api.controlConstructor['range-slider'] = wp.customize.Control.extend({
        ready: function () {
            ...
            initializeSlider('.sparkconstructionlite-slider.desktop-slider', '.desktop-input', control.settings['desktop']);
            initializeSlider('.sparkconstructionlite-slider.tablet-slider', '.tablet-input', control.settings['tablet']);
            initializeSlider('.sparkconstructionlite-slider.mobile-slider', '.mobile-input', control.settings['mobile']);
        }

    });

    // Cssbox (Margin & Padding)
    api.controlConstructor['sparkconstructionlite-cssbox'] = wp.customize.Control.extend({
        ready: function () {
            'use strict';
            var control = this;
            this.container.on(
    ...
    // Tab Control
    api.SparkConstructionLiteTabs = [];
    api.SparkConstructionLiteTab = api.Control.extend({
        ...
        ready: function () {
            ...
            api.SparkConstructionLiteTabs.push(control.id);
        },
    ...
    jQuery.extend(api.controlConstructor, {
        'tab': api.SparkConstructionLiteTab,
    });

    api.bind('ready', function () {
        _.each(api.SparkConstructionLiteTabs, function (id) {
            var control = api.control(id);
            control.toggleActiveControls(0);
        });
    });

    api.controlConstructor['sparkconstructionlite-buttonset'] = wp.customize.Control.extend({
        ready: function () {
    ...
    */
    wp.customize.controlConstructor['sparkconstructionlite-responsive-buttonset'] = wp.customize.Control.extend(
        {
            ready: function () {
            ...
            */
            updateValue: function () {
                ...
                var valueToPush = {};
                control.container.find('.sparkconstructionlite-responsive-buttonset-device-wrap').each(
                    function () {
                        var $ = jQuery;
                        var device = $(this).attr('data-device');
                ...
                var collector = jQuery(control).find('.sparkconstructionlite-responsive-buttonset-collection-value');
                collector.val(JSON.stringify(valueToPush));
                control.setting.set(JSON.stringify(valueToPush));
            },
        }
