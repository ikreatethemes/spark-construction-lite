<?php
$post_sidebar =  get_theme_mod( 'sparkconstructionlite_blog_template_sidebar','right' );

$column =  get_theme_mod( 'sparkconstructionlite_post_column', 2 );

$bspace =  get_theme_mod( 'sparkconstructionlite_blog_post_space', 1 );
			...
			<main id="main" class="site-main">
				<?php if ( have_posts() ) :?>
					<header class="page-header">
						<h3 class="page-title">
							<?php printf( esc_html__( 'Search Results for: %1$s', 'spark-construction-lite' ), '<span>' . get_search_query() . '</span>' ); ?>
						</h3>
					</header>
					...
					the_posts_pagination(
						array(
							...
							'prev_text'          => sprintf(
								...
								esc_html__( 'Prev', 'spark-construction-lite' )
							),
							'next_text'          => sprintf(
								'<span>%s</span> <i class="fa-solid fa-arrow-right-long" aria-hidden="true"></i>',
								esc_html__( 'Next', 'spark-construction-lite' )
							),
							'screen_reader_text' => esc_html__( 'Posts navigation', 'spark-construction-lite' ),
						)
					);
